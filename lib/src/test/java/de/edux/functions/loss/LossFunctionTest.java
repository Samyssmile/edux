...
public class LossFunctionTest {
  ...
  public void testCategoricalCrossEntropy() {
    ...
    assertEquals(
        expectedError,
        LossFunction.CATEGORICAL_CROSS_ENTROPY.calculateError(output, target),
        DELTA);
  }
  ...
  public void testMeanSquaredError() {
    ...
    assertEquals(
        expectedError, LossFunction.MEAN_SQUARED_ERROR.calculateError(output, target), DELTA);
  }
  ...
  public void testMeanAbsoluteError() {
    ...
    assertEquals(
        expectedError, LossFunction.MEAN_ABSOLUTE_ERROR.calculateError(output, target), DELTA);
  }
  ...
  public void testHingeLoss() {
    ...
    double expectedError =
        (Math.max(0, 1 - (-1) * 0.1) + Math.max(0, 1 - (-1) * 0.2) + Math.max(0, 1 - 1 * 0.7)) / 3;
  ...
  public void testSquaredHingeLoss() {
    ...
    double expectedError =
        (Math.pow(Math.max(0, 1 - (-1) * 0.1), 2)
                + Math.pow(Math.max(0, 1 - (-1) * 0.2), 2)
                + Math.pow(Math.max(0, 1 - 1 * 0.7), 2))
            / 3;
    assertEquals(
        expectedError, LossFunction.SQUARED_HINGE_LOSS.calculateError(output, target), DELTA);
  }
  ...
  public void testBinaryCrossEntropy() {
    ...
    assertEquals(
        expectedError, LossFunction.BINARY_CROSS_ENTROPY.calculateError(output, target), DELTA);
  }
}
