...
class DataProcessorTest {
  ...
  void shouldSkipHead() {
    dataProcessor.loadDataSetFromCSV(
        new File("mockpathhere"), ',', SKIP_HEAD, new int[] {0, 1, 2, 4}, 3);
  ...
  void shouldNotSkipHead() {
    dataProcessor.loadDataSetFromCSV(
        new File("mockpathhere"), ',', false, new int[] {0, 1, 2, 4}, 3);
  ...
  void getTargets() {
    when(dataReader.readFile(any(), anyChar())).thenReturn(dummyDataset);
    dataProcessor
        .loadDataSetFromCSV(new File("mockpathhere"), ',', SKIP_HEAD, new int[] {0, 1, 2, 4}, 3)
        .split(0.5);
    ...
    for (int i = 0; i < expectedTargets.length; i++) {
      assertArrayEquals(
          expectedTargets[i], targets[i], "Die Zielzeile " + i + " stimmt nicht überein.");
    }

    Map<String, Integer> classMap = dataProcessor.getClassMap();
    Map<String, Integer> expectedClassMap =
        Map.of(
            "Anna", 0,
            "Nina", 1,
            "Johanna", 2,
  ...
  void getInputs() {
    ...
    assertEquals(
        expectedInputs.length, inputs.length, "Die Anzahl der Zeilen stimmt nicht überein.");

    for (int i = 0; i < expectedInputs.length; i++) {
      assertArrayEquals(
          expectedInputs[i],
          inputs[i],
          "Die Zeile " + i + " entspricht nicht den erwarteten Werten.");
    }
  }
  ...
  void shouldNormalize() {
    ...
    for (int i = 1; i < normalizedDataset.size(); i++) {
      ...
      assertArrayEquals(
          expectedNormalizedValues[i],
          row,
          "Die Zeile " + i + " entspricht nicht den erwarteten normalisierten Werten.");
    }
  }
  ...
  void shouldShuffle() {
    ...
    dataProcessor
        .loadDataSetFromCSV(new File("mockpathhere"), ',', false, new int[] {0, 1, 2, 4}, 3)
        .split(0.5);
